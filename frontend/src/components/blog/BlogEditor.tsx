...
export default function BlogEditor({ mode, initial }: Props) {
  ...
  const publish = async () => {
    if (!post.slug || post.blocks.length === 0 || publishing) return;
    if (!token) {
      setPublishError(`Please log in to ${isEdit ? "save changes" : "publish"}.`);
      return;
    }
    ...
    push(isEdit ? "Changes saved" : "Published");
    ...
    <div className="grid min-h-screen grid-cols-1 lg:grid-cols-2">
      ...
      <div className="flex flex-col gap-4 overflow-y-auto border-r border-neutral-200 bg-neutral-50 p-5">
        ...
        <div className="flex flex-wrap gap-2">
          ...
          >
            {publishing ? (isEdit ? "Saving…" : "Publishing…") : isEdit ? "Save changes → view" : "Publish → view"}
          </button>
          <button
